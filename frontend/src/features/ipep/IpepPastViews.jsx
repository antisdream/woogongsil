...
export function IpepPastLobby({ catalog, selectedExam, setSelectedExam, startPastExam, getText = fallbackText, formatText = fallbackFormatText }) {
        return (
            <section className="learning-study-panel" style={panelStyle}>
                <h3 style={{ color: 'var(--wgs-blue)', fontSize: '24px', margin: '0 0 12px 0' }}>{getText('past_lobby.title', ' 실기 기출문제')}</h3>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '20px' }}>
                    {catalog.map(row => {
                        ...
                        return (
                            <button
                                className="learning-catalog-option" aria-pressed={isSelected}
                                ...
                                style={{ ...baseButtonStyle, background: isSelected ? 'var(--wgs-blue)' : isOpen ? 'var(--wgs-button-muted)' : 'var(--wgs-button-muted)', border: isSelected ? '2px solid var(--wgs-blue)' : '1px solid transparent', opacity: isOpen ? 1 : 0.75 }}
                            >
                ...
                <div style={{ ...panelStyle, background: 'var(--wgs-exam-card)' }}>
                    ...
                    <button
                        className="learning-primary" onClick={() => startPastExam(selectedExam)}
        ...
        return (
            <section className="learning-results" style={{ ...panelStyle, textAlign: 'center' }}>
                ...
                <div style={{ border: `2px solid ${pastSummary.isPass ? '#10b981' : '#ef4444'}`, background: pastSummary.isPass ? 'rgba(16,185,129,0.12)' : 'rgba(239,68,68,0.12)', borderRadius: '14px', padding: '28px', marginBottom: '24px' }}>
                    ...
                    <div style={{ marginTop: '18px', display: 'inline-block', textAlign: 'left', background: 'var(--wgs-card)', padding: '14px 18px', borderRadius: '10px', color: 'var(--wgs-muted)' }}>
                ...
                <div className="learning-table-wrap" style={{ ...panelStyle, textAlign: 'left', overflowX: 'auto', marginBottom: '20px' }}>
                ...
                <div className="ipep-action-row" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    <button className="learning-secondary" onClick={exportPastResultPDF} style={{ ...baseButtonStyle, background: '#10b981', flex: 1 }}>{getText('buttons.export_pdf', 'PDF로 추출')}</button>
                    <button className="learning-secondary" onClick={resetPastToLobby} style={{ ...baseButtonStyle, background: 'var(--wgs-button-muted)', flex: 1 }}>{getText('buttons.back_lobby', '실기 로비로 이동')}</button>
                    <button className="learning-primary" onClick={() => startPastExam(selectedExam)} style={{ ...baseButtonStyle, background: '#3b82f6', flex: 1 }}>{getText('buttons.retry_past', '다시 풀기')}</button>
                </div>
            </section>
        );
