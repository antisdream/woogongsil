...
export function buildWrongHtml({ roomRecord, escapeHtml }) {
    ...
    const sidebarRows = allPrintableItems.map((row) => {
        ...
        return `<tr><td>${escapeHtml(rowNo)}번</td><td>${cells}</td><td>${wrongNames.length}</td></tr>`;
    }).join('');
    ...
    const wrongCards = wrongItems.map((q, idx) => {
        ...
        const questionImageHtml = questionImageSrc
            ? `<div class="html-question-image-wrap"><img class="html-question-image mp-result-img" src="${escapeHtml(questionImageSrc)}" alt="문제 보기 이미지" data-hide-on-error="parent" /></div>`
            : '';
        const explanationImageSrc = getPrintableExplanationImageSrc(q);
        const explanationImageHtml = explanationImageSrc
            ? `<div class="html-question-image-wrap"><img class="html-question-image mp-result-img" src="${escapeHtml(explanationImageSrc)}" alt="해설 이미지" data-hide-on-error="parent" /></div>`
            : '';
...
</style></head><body><button class="print-btn">PDF로 저장/인쇄</button><button class="side-toggle">현황 닫기</button><div class="layout"><main class="main"><h1>${escapeHtml(recordExamMeta.resultTitle)}</h1><p>방 번호 #${escapeHtml(roomRecord.roomCode)} · 전체 채점 문제 ${wrongItems.length}문제</p>${wrongCards}</main><aside class="side"><details open><summary style="font-size:22px;font-weight:900;margin-bottom:12px;cursor:pointer">문제별 정답/오답 현황</summary><table><thead><tr><th>문제번호</th><th>${participantNames.map(escapeHtml).join('</th><th>')}</th><th>틀린수</th></tr></thead><tbody>${sidebarRows}</tbody></table></details></aside></div><div id="mpResultModal" class="mp-result-modal" aria-hidden="true"><div class="mp-result-modal__box"><button type="button" class="mp-result-modal__close" aria-label="닫기">닫기</button><img id="mpResultModalImg" src="" alt="확대 이미지" /></div></div></body></html>`;
}
