...
export function MultiplayerWaitingRoom({ room, activeExamMeta, leaveWaitingRoom, isHost, kickMember, changePassword, startRoom, toggleReady, myMember }) {
    return (
        <section className="learning-question-card" style={cardStyle}>
            <div style={betweenStyle}><h2 className="learning-section-title" style={sectionTitleStyle}>대기방 #{room?.roomCode}</h2><button className="learning-secondary" style={secondaryBtn} onClick={leaveWaitingRoom}>{activeExamMeta.lobbyText}</button></div>
            ...
            <div className="learning-member-grid" style={memberGridStyle}>{(room?.members || []).map((m) => (
            ...
            <div className="learning-action-row" style={buttonRowStyle}>{isHost ? <><button className="learning-secondary" style={secondaryBtn} onClick={changePassword}>비밀번호 변경</button><button className="learning-primary" style={primaryBtn} onClick={startRoom}>시험 시작</button></> : <button className="learning-primary" style={primaryBtn} onClick={toggleReady}>{myMember?.status === 'READY'? '준비 취소' : '준비완료'}</button>}</div>
        </section>
    );
}
    ...
    return (
        <section className="learning-question-card" style={cardStyle}>
            <div style={betweenStyle}>
                <h2 className="learning-section-title" style={sectionTitleStyle}>시험 가이드 및 응시 동의</h2>
            ...
            <div className="learning-agreement-grid" style={agreeGridStyle}>
            ...
            <div className="learning-action-row" style={buttonRowStyle}>
                {/* 제출 없이 대기방으로 돌아가는 경로를 제거합니다. */}
                <button
                    className="learning-primary" style={{ ...primaryBtn, ...(!allAgreed ? disabledBtnStyle : {}) }}
                    disabled={!allAgreed}
...
export function MultiplayerResultView({ result, room, resetToHome }) {
    return (
        <section className="learning-question-card" style={cardStyle}>
            ...
            <div className="learning-subject-grid" style={subjectGridStyle}>{(result?.subjectScores || []).map((s, i) => <div key={i} style={subjectCardStyle}><h3>{s.subjectName || `${i + 1}과목`}</h3><strong>{s.score}점</strong><p>{s.correctCount}/{s.totalCount || 20}</p></div>)}</div>
