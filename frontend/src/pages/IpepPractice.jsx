...
const uniqueRecentIpepList = (list, nextValue, limit) => {
...
const rememberIpepRandomQuestion = (nextQuestion) => {
...
const buildIpepRandomQuery = (subjectCode = 'ALL') => {
    const history = readIpepRandomHistory();
...
function IpepPractice({ setIsExamActive, initialMode = 'lobby' }) {
    ...
    function renderRandomMode() {
        const selectedSubjectName = selectedSubject === 'ALL'? getIpepScreenSetting('random.all_subject_label', '전체 과목 섞기')
            : subjects.find(s => s.subjectCode === selectedSubject)?.subjectName || selectedSubject;

        return (
            <section style={panelStyle}>
                <h3 style={{ color: '#fcd34d', fontSize: '24px', margin: '0 0 12px 0' }}>{getIpepScreenSetting('random.title', ' 실기 문제은행')}</h3>
                <p style={{ color: 'var(--wgs-muted)', lineHeight: 1.7, marginBottom: '18px' }}>
                    {getIpepScreenSetting('random.description', '과목별 랜덤 문제를 풀거나, 전체 과목 섞기를 선택하여 실제 시험처럼 범위를 넓혀 연습할 수 있습니다.')}
                </p>
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '18px' }}>
                    ...
                    >
                        {getIpepScreenSetting('random.all_subject_label', '전체 과목 섞기')}
                    </button>
                ...
                <div style={{ ...panelStyle, background: 'var(--wgs-exam-card)' }}>
                    {randomLoading ? (
                        <p style={{ color: 'var(--wgs-muted)' }}>{getIpepScreenSetting('random.loading', '문제를 불러오는 중입니다...')}</p>
                    ) : randomQuestion ? (
                        <>
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
                                <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>{getIpepScreenSetting('random.badge', '문제은행')}</span>
                                ...
                                <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>{formatIpepSetting('random.grading_policy_badge', '채점유형 {policy}', { policy: randomQuestion.gradingPolicy })}</span>
                            </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                                ...
                                <ErrorReportButton
                                    examType={getIpepScreenSetting('report.exam_type', '실기')} mode={getIpepScreenSetting('report.random_mode', '문제은행')} questionInfo={{
                            ...
                            <textarea
                                ...
                                placeholder={getIpepScreenSetting('form.answer_placeholder', '여기에 실기 답안을 입력해 주세요. 여러 답안은 쉼표 또는 줄바꿈으로 구분하면 됩니다.')} style={{ width: '100%', minHeight: '120px', boxSizing: 'border-box', background: 'var(--wgs-input-bg)', color: 'var(--wgs-text)', border: '1px solid var(--wgs-button-muted)', borderRadius: '10px', padding: '14px', lineHeight: 1.6, resize: 'vertical' }}
                            />
                            ...
                            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '14px' }}>
                                <button onClick={checkRandomAnswer} style={{ ...baseButtonStyle, background: '#3b82f6' }}>{getIpepScreenSetting('buttons.submit_answer', ' 정답 제출')}</button>
                                <button onClick={() => fetchRandomQuestion(selectedSubject)} style={{ ...baseButtonStyle, background: 'var(--wgs-button-muted)' }}>{getIpepScreenSetting('buttons.next_random', ' 다른 문제')}</button>
                            </div>
                                ...
                                <div style={{ marginTop: '16px', padding: '18px', borderRadius: '10px', background: randomResult.isCorrect ? 'rgba(16,185,129,0.16)' : 'rgba(239,68,68,0.16)', border: `1px solid ${randomResult.isCorrect ? '#10b981' : '#ef4444'}` }}>
                                    <h4 style={{ margin: '0 0 12px 0', color: randomResult.isCorrect ? '#10b981' : '#ef4444' }}>
                                        {randomResult.requiresSelfCheck
                                            ? getIpepScreenSetting('result.self_check_needed', '정답 예시 확인이 필요합니다.')
                                            : randomResult.isCorrect
                                                ? formatIpepSetting('result.random_correct', ' 정답입니다! 획득 점수: {score} / {maxScore}', { score: randomResult.score, maxScore: randomResult.maxScore })
                                                : formatIpepSetting('result.random_wrong', ' 오답입니다. 획득 점수: {score} / {maxScore}', { score: randomResult.score, maxScore: randomResult.maxScore })}
                                    </h4>
                                    <div style={{ color: 'var(--wgs-muted)', fontSize: '14px', marginBottom: '8px' }}>{getIpepScreenSetting('result.correct_answer_label', '정답:')}</div>
                                    ...
                                    <div style={{ color: 'var(--wgs-muted)', fontSize: '13px', marginTop: '10px' }}>{formatIpepSetting('result.grading_policy_label', '채점 기준: {policy}', { policy: randomResult.detail?.compareMode || randomResult.gradingPolicy })}</div>
                                    <IpepExplanationImage
                                        question={randomQuestion}
                                        openImageViewer={openImageViewer}
                                        explanationTitle={getIpepScreenSetting('image.explanation_title', ' 해설 이미지')}
                                        explanationAlt={getIpepScreenSetting('image.explanation_alt', '해설 이미지')}
                                        explanationViewerTitle={getIpepScreenSetting('image.explanation_viewer_title', '해설 이미지 크게 보기')}
                                        explanationButtonLabel={getIpepScreenSetting('image.explanation_button', ' 해설 이미지 확대해서 보기')}
                                    />
                                </div>
                            )}
                        </>
                    ) : (
                        <p style={{ color: 'var(--wgs-muted)' }}>{getIpepScreenSetting('random.empty', '표시할 문제가 없습니다.')}</p>
                    )}
                </div>
            </section>
        );
            ...
            >
                <section style={{ ...panelStyle, flex: '1 1 680px', minWidth: 0 }}>
                    ...
                    <textarea
                        ...
                        placeholder={getIpepScreenSetting('form.answer_placeholder', '여기에 실기 답안을 입력해 주세요. 여러 답안은 쉼표 또는 줄바꿈으로 구분하면 됩니다.')} style={{ width: '100%', minHeight: '140px', boxSizing: 'border-box', background: 'var(--wgs-input-bg)', color: 'var(--wgs-text)', border: '1px solid var(--wgs-button-muted)', borderRadius: '10px', padding: '14px', lineHeight: 1.6, resize: 'vertical' }}
                    />
