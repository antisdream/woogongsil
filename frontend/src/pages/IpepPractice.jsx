...
function IpepPractice({ setIsExamActive, initialMode = 'lobby' }) {
    ...
    const [mode, setMode] = useState(() => ['lobby', 'random', 'past'].includes(initialMode) ? initialMode : 'lobby');
    ...
    const saveIpepWrongNotes = async ({ source, wrongQuestions, year = null, session = null }) => {
    ...
    useEffect(() => {
        ...
        if (['lobby', 'random', 'past'].includes(initialMode)) {
            setMode(initialMode);
        }
    ...
    const goIpepMode = (nextMode) => {
        if (pastStep === 'exam') {
            alert('실기 기출 시험 응시 중입니다. 제출 및 채점하기 전에는 실기 로비로 이동할 수 없습니다.');
            return;
        }

        const safeMode = ['lobby', 'random', 'past'].includes(nextMode) ? nextMode : 'lobby';
        setMode(safeMode);
    ...
    useEffect(() => {
        if (pastStep === 'exam' && timeLeft === 0 && !isSubmittingPast) {
            alert('2시간 30분의 실기 시험 시간이 종료되었습니다. 자동으로 제출합니다.');
            submitPastExam(true);
        }
    ...
    async function fetchRandomQuestion(subjectCode = 'ALL') {
        setRandomLoading(true);
        setRandomResult(null);
        setRandomAnswer('');

        try {
            const res = await axios.get(`${API_BASE}/api/ipep/random-question?subjectCode=${encodeURIComponent(subjectCode)}`);
            setRandomQuestion(res.data?.data || null);
        } catch (error) {
            console.error('실기 문제은행 조회 실패:', error);
            setRandomQuestion(null);
            alert('실기 문제은행 문제를 불러오지 못했습니다. 서버 또는 데이터 적재 상태를 확인해 주세요.');
        } finally {
    ...
    async function startPastExam(examRow) {
        ...
        if (Number(examRow.isOpen) !== 1) {
            alert(examRow.noticeMessage || '현재 오픈베타 테스트 중으로, 빠른 시일 내에 추가할 예정입니다.');
            return;
        }
        ...
        try {
            ...
            if (!res.data?.success || !res.data?.isOpen) {
                alert(res.data?.msg || '현재 오픈베타 테스트 중으로, 빠른 시일 내에 추가할 예정입니다.');
                return;
            }

            const questions = Array.isArray(res.data?.data) ? res.data.data : [];
            if (questions.length === 0) {
                alert('해당 회차의 실기 문제가 아직 준비되지 않았습니다.');
                return;
            }
            ...
            alert('실기 기출문제를 불러오는 중 오류가 발생했습니다.');
        }
    }
    ...
    async function gradeOnePastQuestion(question, userAnswer) {
        ...
        if (data.requiresSelfCheck) {
            ...
            const ok = window.confirm(
                `[자기채점 필요]\n\n문제: ${question.questionText}\n\n내 답안:\n${trimmedAnswer || '(미입력)'}\n\n정답 예시:\n${gradedData.correctAnswer || data.correctAnswer || ''}\n\n정답으로 처리할까요?`
            );
    ...
    async function submitPastExam(autoSubmit = false) {
        ...
        if (!autoSubmit) {
            ...
            const confirmMessage = `현재 ${pastQuestions.length}문제 중 ${answeredCount}문제 답안이 입력되었습니다.\n최종 제출 후에는 답안을 수정할 수 없습니다. 제출하시겠습니까?`;
        ...
        try {
            ...
            alert('최종 채점 중 오류가 발생했습니다. 답안은 브라우저 화면에 남아 있으니 다시 시도해 주세요.');
        } finally {
    ...
    function exportPastResultPDF() {
        ...
        const rowsHtml = pastResults.map((row, index) => {
            ...
            return `
                <tr>
                    <td>${qNo}번</td>
                    <td>${escapeHtml(row.userAnswer || '(미입력)')}</td>
                    <td>${escapeHtml(row.correctAnswer || '')}</td>
                    <td>${row.score} / ${row.maxScore}</td>
                    <td class="${isFullCorrect ? 'correct' : 'wrong'}">${isFullCorrect ? 'O' : 'X'}</td>
                </tr>
            `;
        }).join('');
            ...
            ? wrongRows.map((row, index) => {
                ...
                return `
                    <div class="question-box">
                        <div class="q-meta">${q.examYear || selectedExam?.examYear}년 ${q.examSession || selectedExam?.examSession}회차 ${qNo}번 · ${escapeHtml(q.gradingPolicy || '')}</div>
                        <h3>Q. ${escapeHtml(q.questionText || '')}</h3>
                        ${choiceImg ? `<img src="${choiceImg}" class="q-img" onerror=" this.style.display='none'" />` : ''}
                        <p><strong>내 답안:</strong> ${escapeHtml(row.userAnswer || '(미입력)')}</p>
                        <p><strong>정답:</strong> ${escapeHtml(row.correctAnswer || '')}</p>
                        <p><strong>획득 점수:</strong> ${row.score} / ${row.maxScore}</p>
                        ${explanationImg ? `<h4>해설 이미지</h4><img src="${explanationImg}" class="q-img" onerror=" this.style.display='none'" />` : ''}
                    </div>
                `;
            }).join('')
            : '<div class="perfect"> 틀린 문제가 없습니다.</div>';
        ...
        printWindow.document.write(`
            ...
            <body>
                <h1>정보처리기사 실기 결과표</h1>
                <h2>${selectedExam?.examYear || ''}년 ${selectedExam?.examSession || ''}회차</h2>

                <div class="result-box">
                    <div class="score">${pastSummary.totalScore}점 / ${pastSummary.maxScore}점</div>
                    <h2>${pastSummary.isPass ? ' 합격입니다.' : ' 불합격입니다.'}</h2>
                    <p>${escapeHtml(userName)}님 · 정답 처리 ${pastSummary.fullCorrectCount}문제</p>
                    <div class="time-box">
                        <div>시작 일시: <strong>${formatDateTime(startTime)}</strong></div>
                        <div>종료 일시: <strong>${formatDateTime(endTime)}</strong></div>
                        <div>실제 소요 시간: <strong>${formatTime(IPEP_TOTAL_SECONDS - timeLeft)}</strong></div>
                    </div>
                </div>
                ...
                <h2 style="page-break-before: always;">오답 및 해설</h2>
                ${wrongHtml}
    ...
    function openImageViewer(imgPath, title = '이미지') {
    ...
    function renderRandomMode() {
        const selectedSubjectName = selectedSubject === 'ALL'? '전체 과목 섞기'
            : subjects.find(s => s.subjectCode === selectedSubject)?.subjectName || selectedSubject;

        return (
            <section style={panelStyle}>
                <h3 style={{ color: '#fcd34d', fontSize: '24px', margin: '0 0 12px 0' }}> 실기 문제은행</h3>
                <p style={{ color: 'var(--wgs-muted)', lineHeight: 1.7, marginBottom: '18px' }}>
                    과목별 랜덤 문제를 풀거나, 전체 과목 섞기를 선택하여 실제 시험처럼 범위를 넓혀 연습할 수 있습니다.
                </p>

                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '18px' }}>
                    <button
                        onClick={() => setSelectedSubject('ALL')}
                        style={{ ...baseButtonStyle, background: selectedSubject === 'ALL'? '#3b82f6' : 'var(--wgs-button-muted)' }}
                    >
                        전체 과목 섞기
                    </button>
                    {subjects.map(subject => (
                        <button
                            ...
                            style={{ ...baseButtonStyle, background: selectedSubject === subject.subjectCode ? '#3b82f6' : 'var(--wgs-button-muted)' }}
                        >
                ...
                <div style={{ ...panelStyle, background: 'var(--wgs-exam-card)' }}>
                    {randomLoading ? (
                        <p style={{ color: 'var(--wgs-muted)' }}>문제를 불러오는 중입니다...</p>
                    ) : randomQuestion ? (
                        <>
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
                                <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>문제은행</span>
                                <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>{selectedSubjectName}</span>
                                <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>채점유형 {randomQuestion.gradingPolicy}</span>
                            </div>

                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                                <h4 style={{ color: 'var(--wgs-text)', fontSize: '18px', lineHeight: 1.7, margin: 0 }}>{randomQuestion.questionText}</h4>
                                {/* 실기 문제은행의 현재 단답형 문항을 관리자에게 즉시 신고하는 버튼입니다. */}
                                <ErrorReportButton
                                    examType="실기" mode="문제은행" questionInfo={{
                            ...
                            <IpepQuestionImages question={randomQuestion} openImageViewer={openImageViewer} />

                            <textarea
                                value={randomAnswer}
                                onChange={(e) => setRandomAnswer(e.target.value)}
                                placeholder="여기에 실기 답안을 입력해 주세요. 여러 답안은 쉼표 또는 줄바꿈으로 구분하면 됩니다." style={{ width: '100%', minHeight: '120px', boxSizing: 'border-box', background: 'var(--wgs-input-bg)', color: 'var(--wgs-text)', border: '1px solid var(--wgs-button-muted)', borderRadius: '10px', padding: '14px', lineHeight: 1.6, resize: 'vertical' }}
                            />

                            {/* 필기 페이지의 연습장과 동일한 컴포넌트를 실기 문제은행에도 추가합니다. */}
                            <button
                            ...
                            >
                                 연습장 {isDrawingOpen ? '닫기' : '열기'}
                            </button>
                            {isDrawingOpen && <DrawingBoard />}

                            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '14px' }}>
                                <button onClick={checkRandomAnswer} style={{ ...baseButtonStyle, background: '#3b82f6' }}> 정답 제출</button>
                                <button onClick={() => fetchRandomQuestion(selectedSubject)} style={{ ...baseButtonStyle, background: 'var(--wgs-button-muted)' }}> 다른 문제</button>
                            </div>
                            ...
                            {randomResult && (
                                <div style={{ marginTop: '16px', padding: '18px', borderRadius: '10px', background: randomResult.isCorrect ? 'rgba(16,185,129,0.16)' : 'rgba(239,68,68,0.16)', border: `1px solid ${randomResult.isCorrect ? '#10b981' : '#ef4444'}` }}>
                                    <h4 style={{ margin: '0 0 12px 0', color: randomResult.isCorrect ? '#10b981' : '#ef4444' }}>
                                        {randomResult.requiresSelfCheck ? '정답 예시 확인이 필요합니다.' : randomResult.isCorrect ? ` 정답입니다! 획득 점수: ${randomResult.score} / ${randomResult.maxScore}` : ` 오답입니다. 획득 점수: ${randomResult.score} / ${randomResult.maxScore}`}
                                    </h4>
                                    ...
                                    <div style={{ color: 'var(--wgs-muted)', fontSize: '13px', marginTop: '10px' }}>채점 기준: {randomResult.detail?.compareMode || randomResult.gradingPolicy}</div>
                                    <IpepExplanationImage question={randomQuestion} openImageViewer={openImageViewer} />
                                </div>
                            )}
                        </>
                    ) : (
                        <p style={{ color: 'var(--wgs-muted)' }}>표시할 문제가 없습니다.</p>
                    )}
                </div>
            </section>
        );
            ...
            >
                <section style={{ ...panelStyle, flex: '1 1 680px', minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', borderBottom: '1px solid var(--wgs-border)', paddingBottom: '12px', marginBottom: '18px' }}>
                        <strong style={{ color: 'var(--wgs-blue-soft)', fontSize: '18px' }}>{selectedExam?.examYear}년 {selectedExam?.examSession}회차</strong>
                        <span style={{ color: 'var(--wgs-muted)' }}>{currentIndex + 1} / {pastQuestions.length}</span>
                    </div>

                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
                        <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>기출문제</span>
                        <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>{selectedExam?.examYear}년 {selectedExam?.examSession}회차</span>
                        <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>{getQuestionNo(currentQuestion, currentIndex)}번</span>
                        <span style={{ border: '1px solid #3b82f6', color: 'var(--wgs-blue-soft)', borderRadius: '20px', padding: '5px 10px', fontSize: '12px', fontWeight: '900' }}>채점유형 {currentQuestion.gradingPolicy}</span>
                    </div>
                    ...
                    <div className="exam-question-title-row" style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
                        ...
                        <ErrorReportButton
                            examType="실기" mode="기출문제" questionInfo={{
                                year: selectedExam?.examYear,
                    ...
                    <IpepQuestionImages question={currentQuestion} openImageViewer={openImageViewer} />

                    <textarea
                        value={pastAnswers[currentQuestion.questionId] || ''}
                        onChange={(e) => updatePastAnswer(currentQuestion.questionId, e.target.value)}
                        placeholder="여기에 실기 답안을 입력해 주세요. 여러 답안은 쉼표 또는 줄바꿈으로 구분하면 됩니다." style={{ width: '100%', minHeight: '140px', boxSizing: 'border-box', background: 'var(--wgs-input-bg)', color: 'var(--wgs-text)', border: '1px solid var(--wgs-button-muted)', borderRadius: '10px', padding: '14px', lineHeight: 1.6, resize: 'vertical' }}
                    />
                    ...
                    <div className="ipep-action-row" style={{ display: 'flex', gap: '12px', marginTop: '18px' }}>
                        ...
                        >
                            {isSubmittingPast ? '채점 중...' : '제출 및 채점하기'}
                        </button>
                        <button
                    ...
                    <div style={{ marginTop: '16px', color: 'var(--wgs-muted)', fontSize: '13px' }}>
                        현재 입력 완료: {answeredCount} / {pastQuestions.length}문제
                    </div>
                </section>
                ...
                <aside className="ipep-past-exam-omr" style={{ flex: '0 0 260px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ background: 'var(--wgs-panel)', padding: '16px', border: '1px solid var(--wgs-border)', borderTopLeftRadius: '12px', borderTopRightRadius: '12px', textAlign: 'center' }}>
                        <div style={{ color: 'var(--wgs-blue-soft)', fontWeight: '900', marginBottom: '6px' }}>남은 시간</div>
                    ...
                    <div style={{ background: 'var(--wgs-card)', border: '1px solid var(--wgs-border)', borderTop: 'none', borderBottomLeftRadius: '12px', borderBottomRightRadius: '12px', padding: '16px', overflowY: 'auto' }}>
                        <h4 style={{ color: 'var(--wgs-text)', textAlign: 'center', margin: '0 0 14px 0' }}>진행 현황 (OMR)</h4>
    ...
    return (
        <div className="ipep-page exam-page wgs-typography-scope" style={pageBoxStyle}>
            ...
            <h2 className="wgs-page-title" style={{ color: 'var(--wgs-title)', fontSize: '28px', fontWeight: '900', lineHeight: 1.35, margin: '0 0 12px 0' }}> 정보처리기사 실기문제</h2>
            <p style={{ color: 'var(--wgs-text)', fontSize: '16px', fontWeight: '500', lineHeight: 1.7, margin: '0 0 24px 0' }}>
                실기 문제은행은 즉시 채점 방식으로 연습할 수 있으며,<br />실기 기출문제는 실제 시험처럼 최종 제출 후 결과를 확인하는 방식으로 제공됩니다.
            </p>
            ...
            {pastStep === 'exam'? (
                ...
                <>
                    <IpepModeButtons mode={mode} goIpepMode={goIpepMode} />
                    <IpepGuide />
                    {mode === 'random'? renderRandomMode() : <IpepPastLobby
                        catalog={catalog}
                        selectedExam={selectedExam}
