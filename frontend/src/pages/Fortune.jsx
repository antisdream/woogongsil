...
const TIME_OPTIONS = [
    { label: "선택 (시간을 모름)", value: "unknown" },
    { label: "자시 (23:30 ~ 01:29)", value: "00:00" }, { label: "축시 (01:30 ~ 03:29)", value: "02:00" },
    { label: "인시 (03:30 ~ 05:29)", value: "04:00" }, { label: "묘시 (05:30 ~ 07:29)", value: "06:00" },
    { label: "진시 (07:30 ~ 09:29)", value: "08:00" }, { label: "사시 (09:30 ~ 11:29)", value: "10:00" },
    { label: "오시 (11:30 ~ 13:29)", value: "12:00" }, { label: "미시 (13:30 ~ 15:29)", value: "14:00" },
    { label: "신시 (15:30 ~ 17:29)", value: "16:00" }, { label: "유시 (17:30 ~ 19:29)", value: "18:00" },
    { label: "술시 (19:30 ~ 21:29)", value: "20:00" }, { label: "해시 (21:30 ~ 23:29)", value: "22:00" }
];
...
const Fortune = () => {
    ...
    const handleIndivSubmit = async (e) => {
        e.preventDefault();
        // 이름 검증 적용
        if (!validateName(indiv.name)) return alert("이름이 정확하지 않습니다, 올바른 이름을 입력해주세요!");
        if (indiv.bYear === '연도' || indiv.bMonth === '월' || indiv.bDay === '일') return alert("생년월일을 모두 선택해주세요!");
    ...
    const handleCoupleSubmit = async (e) => {
        ...
        if (couple.p1.name === couple.p2.name && 
            ...
            return alert("올바른 상대방 정보를 입력 및 선택해주세요.");
        }
    ...
    const renderSajuBox = (sajuObj, title) => (
        <div style={{ flex: 1, background: 'var(--wgs-practice-toggle-bg)', padding: '20px', borderRadius: '12px', border: '1px solid var(--wgs-border)' }}>
            <h4 style={{ textAlign: 'center', color: 'var(--wgs-title)', marginTop: 0 }}>{title} 사주</h4>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '5px' }}>
                {[['시', sajuObj.hour], ['일', sajuObj.day], ['월', sajuObj.month], ['년', sajuObj.year]].map(([label, pillar], idx) => (
    ...
    const DateSelector = ({ data, setter }) => (
        <div style={{ display: 'flex', gap: '5px' }}>
            <select value={data.bYear} onChange={(e) => setter({...data, bYear: e.target.value})} style={{ flex: 1.5, padding: '10px', borderRadius: '8px', border: '1px solid var(--wgs-border)', background: 'var(--wgs-input-bg)', color: 'white' }}>
                <option value="연도">연도</option>{YEARS.map(y => <option key={y} value={y}>{y}년</option>)}
            </select>
            <select value={data.bMonth} onChange={(e) => setter({...data, bMonth: e.target.value})} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid var(--wgs-border)', background: 'var(--wgs-input-bg)', color: 'white' }}>
                <option value="월">월</option>{MONTHS.map(m => <option key={m} value={m}>{m}월</option>)}
            </select>
            <select value={data.bDay} onChange={(e) => setter({...data, bDay: e.target.value})} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid var(--wgs-border)', background: 'var(--wgs-input-bg)', color: 'white' }}>
                <option value="일">일</option>{DAYS.map(d => <option key={d} value={d}>{d}일</option>)}
            </select>
        </div>
    );
        ...
        <div style={{ maxWidth: '800px', margin: '30px auto', color: 'white', fontFamily: 'var(--wgs-font-body)' }}>
            {step === 1 && (
                <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
                    <button onClick={() => setActiveTab('individual')} style={{ flex: 1, padding: '15px', background: activeTab === 'individual'? '#3b82f6' : 'var(--wgs-button-muted)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '16px' }}> 오늘의 운세?</button>
                    <button onClick={() => setActiveTab('couple')} style={{ flex: 1, padding: '15px', background: activeTab === 'couple'? '#ec4899' : 'var(--wgs-button-muted)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '16px' }}> 오늘의 궁합?</button>
                </div>
            )}

            {step === 1 && activeTab === 'individual' && (
                <div style={{ background: 'var(--wgs-button-muted)', padding: '40px', borderRadius: '12px' }}>
                    <h2 style={{ color: '#fcd34d', textAlign: 'center', marginTop: 0 }}> 오늘의 운세 정보 입력</h2>
                    <form onSubmit={handleIndivSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginTop: '20px' }}>
                        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
                            ...
                            <div style={{ flex: '1 1 150px' }}><label style={{ color: 'var(--wgs-muted)', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>성별</label>
                                <div style={{ display: 'flex', gap: '10px' }}>
                                    <button type="button" onClick={() => setIndiv({...indiv, gender: 'male'})} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: `2px solid ${indiv.gender === 'male'? '#3b82f6' : 'var(--wgs-border)'}`, background: indiv.gender === 'male'? 'rgba(59, 130, 246, 0.2)' : 'var(--wgs-input-bg)', color: 'white' }}>남</button>
                                    <button type="button" onClick={() => setIndiv({...indiv, gender: 'female'})} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: `2px solid ${indiv.gender === 'female'? '#ef4444' : 'var(--wgs-border)'}`, background: indiv.gender === 'female'? 'rgba(239, 68, 68, 0.2)' : 'var(--wgs-input-bg)', color: 'white' }}>여</button>
                                </div>
                            </div>
                        </div>
                        <div><label style={{ color: 'var(--wgs-muted)', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>생년월일 (양력)</label><DateSelector data={indiv} setter={setIndiv} /></div>
                        <div><label style={{ color: 'var(--wgs-muted)', fontWeight: 'bold', display: 'block', marginBottom: '8px' }}>태어난 시간 (선택)</label>
                        ...
                        <button type="submit" style={{ padding: '18px', background: '#10b981', color: 'white', border: 'none', borderRadius: '8px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer', marginTop: '10px' }}>결과 확인하기</button>
                    </form>
                </div>
            )}
            ...
            {step === 1 && activeTab === 'couple' && (
                <div style={{ background: 'var(--wgs-button-muted)', padding: '30px', borderRadius: '12px' }}>
                    <h2 style={{ color: '#f9a8d4', textAlign: 'center', marginTop: 0 }}> 오늘의 궁합 정보 입력</h2>
                    <form onSubmit={handleCoupleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '30px', marginTop: '20px' }}>
                        {[ {id: 'p1', label: '나의 정보', color: '#3b82f6'}, {id: 'p2', label: '상대방 정보', color: '#ef4444'} ].map(person => (
                            <div key={person.id} style={{ padding: '20px', background: 'var(--wgs-practice-toggle-bg)', borderRadius: '10px', borderLeft: `4px solid ${person.color}` }}>
                                ...
                                <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', marginBottom: '15px' }}>
                                    <div style={{ flex: '2 1 150px' }}><label style={{ color: 'var(--wgs-subtle)', fontSize: '13px', display: 'block', marginBottom: '5px' }}>이름</label><input type="text" placeholder="이름 입력" value={couple[person.id].name} onChange={e => setCouple({...couple, [person.id]: {...couple[person.id], name: e.target.value}})} style={{ width: '100%', boxSizing: 'border-box', padding: '10px', borderRadius: '8px', border: '1px solid var(--wgs-border)', background: 'var(--wgs-input-bg)', color: 'white' }} /></div>
                                    <div style={{ flex: '1 1 100px' }}><label style={{ color: 'var(--wgs-subtle)', fontSize: '13px', display: 'block', marginBottom: '5px' }}>성별</label><select value={couple[person.id].gender} onChange={e => setCouple({...couple, [person.id]: {...couple[person.id], gender: e.target.value}})} style={{ width: '100%', boxSizing: 'border-box', padding: '10px', borderRadius: '8px', border: '1px solid var(--wgs-border)', background: 'var(--wgs-input-bg)', color: 'white' }}><option value="male">남성</option><option value="female">여성</option></select></div>
                                </div>
                                <div style={{ marginBottom: '15px' }}><label style={{ color: 'var(--wgs-subtle)', fontSize: '13px', display: 'block', marginBottom: '5px' }}>생년월일 (양력)</label><DateSelector data={couple[person.id]} setter={(newData) => setCouple({...couple, [person.id]: newData})} /></div>
                                <div><label style={{ color: 'var(--wgs-subtle)', fontSize: '13px', display: 'block', marginBottom: '5px' }}>태어난 시간</label><select value={couple[person.id].birthtime} onChange={e => setCouple({...couple, [person.id]: {...couple[person.id], birthtime: e.target.value}})} style={{ width: '100%', boxSizing: 'border-box', padding: '10px', borderRadius: '8px', border: '1px solid var(--wgs-border)', background: 'var(--wgs-input-bg)', color: 'white' }}>{TIME_OPTIONS.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
                            </div>
                        ))}
                        <button type="submit" style={{ padding: '18px', background: '#ec4899', color: 'white', border: 'none', borderRadius: '8px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>궁합 확인하기</button>
                    </form>
                </div>
            )}
            ...
            {step === 2 && (
                <div style={{ background: 'var(--wgs-button-muted)', padding: '60px 20px', borderRadius: '12px', textAlign: 'center' }}>
                    ...
                    <h2 style={{ color: activeTab === 'couple'? '#f9a8d4' : '#fcd34d', marginTop: '20px' }}>만세력을 세우는 중입니다...</h2>
            ...
            {step === 3 && activeTab === 'individual' && result && (
                <div style={{ animation: 'fadeIn 0.8s' }}>
                    <div style={{ background: 'var(--wgs-practice-toggle-bg)', padding: '30px', borderRadius: '12px', marginBottom: '20px' }}>
                        <h2 style={{ textAlign: 'center', color: '#fcd34d', marginTop: 0 }}> {result.name}님의 사주 명식</h2>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px', padding: '15px', background: 'var(--wgs-input-bg)', borderRadius: '8px' }}>
                            <div>격국: <strong style={{ color: '#fcd34d' }}>{result.gyeokguk}</strong></div>
                            <div>용신: <strong style={{ color: getElementColor(result.yongsin) }}>{result.yongsin}</strong></div>
                        </div>
                    </div>
                    <div style={{ background: 'var(--wgs-button-muted)', padding: '20px', borderRadius: '12px', borderLeft: '5px solid #10b981', marginBottom: '15px' }}>
                        <h3 style={{ color: '#34d399', marginTop: 0 }}> 학업운 (오늘의 신살: {result.todaySinsal})</h3>
                        <p style={{ margin: 0 }}>{result.examLuck}</p>
                    </div>
                    <div style={{ background: 'var(--wgs-button-muted)', padding: '20px', borderRadius: '12px' }}>
                        <h3 style={{ color: '#f472b6', marginTop: 0 }}> 종합 조언</h3>
                        <p style={{ margin: 0, color: 'var(--wgs-muted)' }}>{result.totalLuck}</p>
                    </div>
                    <button onClick={() => setStep(1)} style={{ width: '100%', padding: '15px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', marginTop: '20px', fontWeight: 'bold' }}>다시 하기</button>
                </div>
            )}
            ...
            {step === 3 && activeTab === 'couple' && result && (
                <div style={{ animation: 'fadeIn 0.8s' }}>
                    <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                        <h2 style={{ color: '#f9a8d4', margin: 0 }}> {result.p1.name} & {result.p2.name} 궁합 지수</h2>
                        <div style={{ fontSize: '50px', fontWeight: 'bold', color: result.score >= 80 ? '#10b981' : '#f59e0b', marginTop: '10px' }}>{result.score}점</div>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {[
                            { title: '1. 겉궁합 (띠와 연주)', text: result.details.out, color: 'var(--wgs-blue)' },
                            { title: '2. 속궁합 (일간과 일지)', text: result.details.in, color: '#c084fc' },
                            { title: '3. 조후와 억부 (오행 밸런스)', text: result.details.balance, color: '#f472b6' },
                            { title: '4. 운의 흐름 (대운 일치)', text: result.details.flow, color: '#fbbf24' }
                        ].map((item, i) => (
                    ...
                    <button onClick={() => setStep(1)} style={{ width: '100%', padding: '15px', background: '#ec4899', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', marginTop: '20px', fontWeight: 'bold' }}>다른 궁합 보기</button>
