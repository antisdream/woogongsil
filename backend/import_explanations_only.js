  ...
  try {
    const [colRows] = await conn.query(`
      ...
      WHERE table_schema = DATABASE()
        ...
        AND column_name IN ('question_id', 'correct_label', 'explanation_text', 'explanation_img')
    ...
    const [[matchRow]] = await conn.query(`
    ...
    const [[beforeGuard]] = await conn.query(`
      SELECT
        COUNT(*) AS cnt,
    ...
    const [[afterGuard]] = await conn.query(`
      SELECT
        COUNT(*) AS cnt,
        COALESCE(SUM(CRC32(CONCAT_WS('|', question_id, COALESCE(correct_label, ''), COALESCE(explanation_img, '')))), 0) AS guard_checksum
      FROM answers
