...
function registerUserRoutes(options = {}) {
    ...
    if (!app || !pool || !getUserById || !formatDateOnly || !getKSTDateTime || typeof validateRealtimeSession !== 'function') {
        throw new Error('registerUserRoutes requires app, pool, getUserById, formatDateOnly, getKSTDateTime, validateRealtimeSession, and legalConsentService.');
    }
        ...
        try {
            ...
            const [fortuneHistory] = await pool.query(
                `SELECT id, DATE_FORMAT(time, '%Y-%m-%d %H:%i:%s') AS time,
                        type, data, storage_version AS storageVersion
                 FROM wgs_fortune_history
                 WHERE userId = ? AND storage_version = 'RESULT_ONLY_V1'
                 ORDER BY time DESC
                 LIMIT 10`,
                [id]
            );
    ...
    app.post('/api/user/delete', async (req, res) => {
        ...
        try {
            ...
            await getLegalConsentService().markUserEvidenceForRetention(connection, id);
