...
- 공개 웹 응답, 미승인 기기의 관리자 HTML·API 404, 승인 기기의 로그인 HTML과 비로그인 API 401, 종료한 공개 접속자 API, 방문 기록 응답과 공개 합계의 필드 경계를 확인.
...
2.4.2부터 [관리자 기기 인증 설정](./ADMIN_DEVICE_ACCESS.md)이 필요합니다. 두 기기 등록과 로컬 TLS 검증을 마친 뒤 서버의 전체 백업을 검증하고, 저장소 밖 정책 파일·공개 인증서와 Nginx의 인증서 전달 설정을 준비합니다. 개인 키·설치 암호는 서버나 GitHub에 넣지 않습니다. 설정이 누락되면 관리자 접근과 배포 건강 검사가 실패하므로 공개 접근으로 우회하지 말고 서버의 복구 경로를 사용합니다.
